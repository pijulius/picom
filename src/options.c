...
static const struct picom_option picom_options[] = {
    ...
    [345] = {"frame-opacity-for-same-colors", ENABLE(frame_opacity_for_same_colors)         , "Enable frame opacity for colors that match the frame"},
    [346] = {"frame-opacity-for-same-colors-tolerance", FLOAT(frame_opacity_for_same_colors_tolerance, 0, 1), "Tolerance for similar colors (0 exact match, 1 all colors, default 0.5)"},
    [347] = {"frame-opacity-for-same-colors-multiplier", INTEGER(frame_opacity_for_same_colors_multiplier, 0, INT_MAX), "Make different colors opaque by a factor of x (default 5)"},
    [348] = {"inner-border-width"          , INTEGER(inner_border_width, 0, INT_MAX)        , "Draw border inside windows with the defined brightness (default 0)"},
    [349] = {"inner-border-brightness"     , FLOAT(inner_border_brightness, 0, INFINITY)    , "Brightness for the inner borders (default 0.89)"},
